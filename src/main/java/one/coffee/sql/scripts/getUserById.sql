...
FROM (
    SELECT users.userId AS user1Id, users.city AS city1, users.stateId AS state1Id, userConnections.id AS connection1Id, user2Id
    FROM users
        LEFT JOIN userStates ON userStates.stateId = users.stateId
